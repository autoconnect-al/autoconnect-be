...
/**
 * Checks if a post is within the last X days
 * @param createdTime - Post creation time (ISO string or Unix timestamp in seconds)
 * @param days - Number of days to check back
 * @returns true if post is within X days, false otherwise
 */
export function isWithinDays(
  createdTime: string | number | undefined | null,
  days: number,
): boolean {
  if (!createdTime || days < 0) return false;
